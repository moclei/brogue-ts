...
export function buildMovementContext(): PlayerMoveContext {
    ...
    return {
        ...
        // ── Ally/captive ──────────────────────────────────────────────────────
        freeCaptive: (m) => freeCaptiveFn(m, { player, pmap, refreshDungeonCell, monsterAtLoc, cellHasTerrainFlag, message: io.message, monsterName: buildMonsterNameHelper(player), demoteMonsterFromLeadership: (x) => demoteMonsterFromLeadershipFn(x, monsters), makeMonsterDropItem: (x) => { if (x.carriedItem) { const item = x.carriedItem; item.loc = { ...x.loc }; floorItems.push(item); pmap[x.loc.x][x.loc.y].flags |= TileFlag.HAS_ITEM; x.carriedItem = null; refreshDungeonCell(x.loc); } } }),
