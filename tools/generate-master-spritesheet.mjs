...
const SHEET_PATHS = {
  ...
  TheRoguelikeSolid: "the-roguelike/the-roguelike-solid.png",
  TheRoguelikeRemake: "the-roguelike/the-roguelike-re-alpha.png",
  TheRoguelikeNatural: "the-roguelike/the-roguelike-natural.png",
  TheRoguelikeNaturalTileset: "the-roguelike/the-roguelike-natural-tileset.png",
  TheRoguelikeNaturalPropsIcons:
    "the-roguelike/the-roguelike-natural-props-icons.png",
...
async function main() {
  ...
  function extractTile(sheetData, tileX, tileY) {
    ...
    for (let row = 0; row < TILE_SIZE; row++) {
      ...
      const copyLen = Math.min(
        TILE_SIZE * channels,
        info.width * channels - srcStartX * channels,
      );
  ...
  console.log(
    `  ✓ ${pngPath} (${SHEET_W}×${SHEET_H}, ${composites.length} sprites)`,
  );
