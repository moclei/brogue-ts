...
1. **`master-spritesheet.png`** — a packed PNG for all standard (16×16) TileType and DisplayGlyph sprites.
2. **`master-{stride}.png`** — per-stride atlases for hi-res glyphs (e.g. `master-32.png`, `master-64.png`). Generated automatically when a glyph is assigned from a sheet with a non-16 stride or via a multi-tile rect selection.
3. **`sprite-manifest.json`** — maps enum names to grid coordinates and dimensions in the appropriate atlas. Hi-res entries include `w`, `h` (pixels), and `sheet` (atlas key). The game's `glyph-sprite-map.ts` reads this at startup.
4. **`assignments.json`** — single source of truth for all sprite config: `sheets` (`Record<string, string>` mapping atlas key → filename), `tiletype`/`glyph` (source refs with optional `w`/`h` in tile units), and `autotile` (per-group sheet path + format). The game reads this directly at runtime.
...
- **Add sheets:** `+` button on each group opens a dialog to pick a PNG, set a key, category, and optional stride (px per tile; default 16), and upload it into `rogue-ts/assets/tilesets/`
...
- Grid lines adapt to the sheet's declared stride (16, 32, 64 px, etc.)
- Assignment markers show which tiles are already used, spanning their full `w×h` tile rect
- **TileType mode:** click to select a single tile
- **DisplayGlyph mode:** click-drag to select a multi-tile rect; `SelectionBar` shows e.g. "2×2 tiles (32 px)"
...
│   └── generate.ts               — sharp-based atlas compositing; groups by output stride, writes master + per-stride atlases
└── components/
...
- `tileset-loader.ts` — loads all atlases (master + per-stride + autotile) from URLs built by `buildSheetUrls()`; `MAX_SPRITE_SIZE = 64` caps tint canvas allocation
- `glyph-sprite-map.ts` — reads `sprite-manifest.json` + `assignments.json` to build TileType/Glyph/autotile sprite maps; populates `srcW`/`srcH` on hi-res `SpriteRef`s
- `sprite-renderer.ts` — uses per-ref `srcW`/`srcH` in `drawSpriteTinted` and `precreateBitmaps`; bitmap cache keys include dimensions for hi-res refs
